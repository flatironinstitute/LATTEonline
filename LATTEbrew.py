...
def brew_LATTE(tic, random_number, db, lc_paths, indir, outpath, syspath, transit_list, simple, BLS, model, save, DV, sectors, sectors_all, alltime, allflux, allflux_err, all_md, alltimebinned, allfluxbinned, allx1, allx2, ally1, ally2, alltime12, allfbkg, start_sec, end_sec, in_sec, upper_axis_lim_final, lower_axis_lim_final, tessmag, teff, srad, ra, dec, input_numax, input_analysis_window, url_list, args):
	...
	if (save == True) or (DV == True):
		...
		newpath = '{}/{}_rn{}'.format(outpath,tic, random_number)

		if not exists(newpath):
			os.makedirs(newpath)

		# save the data used as a text file - these often come in use later for a quick re-analysis.
		with open('{}/{}_rn{}/{}_data.txt'.format(outpath, tic, random_number, tic), "w") as f:
		...
		if len(transit_list) != 0:
			if model == True:
				with open('{}/{}_rn{}/{}_data_pyaneti.dat'.format(outpath, tic,random_number, tic), "w") as f:
					writer = csv.writer(f, delimiter='\t')
					writer.writerow(['#time', 'flux', 'flux_err'])
				...
				if (len(transit_list) > 1) and ((transit_list[1] - transit_list[0]) < 2): # if there are LOTS of transit events on short period (if so it's probably a TOI but let's keep it here as a condition)
					with open('{}/{}_rn{}/{}_data_pyaneti.dat'.format(outpath, tic, random_number, tic), "a") as f:
						writer = csv.writer(f, delimiter='\t')
	...
	if len(transit_list) != 0: # this is always the case unless the asteroseismic only option ws chosen
		...
		utils.plot_centroid(tic, random_number, outpath, alltime12, allx1, ally1, allx2, ally2, transit_list, args)
		# plot the background flux at the time of the transit event.
		utils.plot_background(tic, random_number, outpath, alltime, allfbkg, transit_list, args)
		...
		#TESS_unbinned_t_l, TESS_binned_t_l, small_binned_t_l, TESS_unbinned_l, TESS_binned_l, small_binned_l, tpf_list = utils.download_tpf_lightkurve(indir, random_number, transit_list, sectors, tic)

		#print ("\n Start downloading of the target pixel files - this can take a little while (up to a minute) as the files are large \n")

		X1_list, X4_list, oot_list, intr_list, bkg_list, apmask_list, arrshape_list, t_list, T0_list, tpf_filt_list,TESS_unbinned_t_l, TESS_binned_t_l, small_binned_t_l, TESS_unbinned_l, TESS_binned_l, small_binned_l, tpf_list = utils.download_tpf(indir, outpath, random_number, transit_sec, transit_list, tic, lc_paths)
		...
		if (TESS_unbinned_t_l[0] != -111):
			...
			utils.plot_aperturesize(tic, random_number, outpath, TESS_unbinned_t_l, TESS_binned_t_l, small_binned_t_l, TESS_unbinned_l, TESS_binned_l, small_binned_l, transit_list, args)
			...
			if args.mpi == False:
				test_astroquery, _, _, mstar, vmag, logg, plx, c_id = utils.plot_TESS_stars(tic,random_number, outpath, transit_sec, tpf_list, args)

				if test_astroquery == -111:
					tessmag, teff, srad, mstar, vmag, logg, plx, c_id = utils.plot_TESS_stars_not_proj(tic,random_number, outpath, transit_list, transit_sec, tpf_list, args)
					args.mpi = True

			else:
				test_astroquery, _, _, mstar, vmag, logg, plx, c_id = utils.plot_TESS_stars_not_proj(tic,random_number, outpath, transit_list, transit_sec, tpf_list, args)
			...
			#X1_list, X4_list, oot_list, intr_list, bkg_list, apmask_list, arrshape_list, t_list, T0_list, tpf_filt_list = utils.download_tpf_mast(indir, random_number, transit_sec, transit_list, tic)
			...
			'''
			if args.north == True:
				utils.plot_in_out_TPF_proj(tic, random_number, outpath, X4_list, oot_list, t_list, intr_list, T0_list, tpf_filt_list, tpf_list, args)
				#print ("In and out of aperture flux comparison with reprojection... done. ")

			else:
				utils.plot_in_out_TPF(tic, random_number, outpath, X4_list, oot_list, t_list, intr_list, T0_list, tpf_filt_list, args)
			...
			utils.plot_pixel_level_LC(tic, random_number, outpath, X1_list, X4_list, oot_list, intr_list, bkg_list, tpf_list, apmask_list, arrshape_list, t_list, T0_list, args)
		...
		#
		#	if save == True:
		#		plt.savefig('{}/{}_rn{}/{}_phase_folded.png'.format(indir, tic, random_number, tic), format='png')
		#
		...
		if args.FFI == False:
			...
			alltime_nn, allflux_nn, all_md_nn, alltimebinned_nn, allfluxbinned_nn,outtics,tessmag_list, distance = utils.download_data_neighbours(indir, db, transit_sec[0], ticids, distance)

			# plot the LCs
			utils.plot_nn(tic, random_number, outpath,alltime_nn, allflux_nn, alltimebinned_nn, allfluxbinned_nn, transit_list, outtics, tessmag_list, distance, args)
		...
		if BLS == True:
			...
			bls_stats1, bls_stats2 = utils.data_bls(tic, random_number, outpath, alltime, allflux, allfluxbinned, alltimebinned, args)
			#print ("done.")
		...
		utils.plot_full_md_notransits(tic, random_number, outpath, alltime, allflux, all_md, alltimebinned, allfluxbinned, upper_axis_lim_final, lower_axis_lim_final, args)
		target_ra = ra
		target_dec = dec
		tpf_corrupt = False
	...
	mass_ast, radius_ast, logg_ast, numax, deltanu = utils.plot_periodogram(tic, random_number, outpath, alltime, allflux, teff, -999, -999, args)
	#print ("done.")

	# ------------
	# stellar evolutionary tracks
	#print ("Evolutionary tracks plot...", end =" ")
	utils.eep_target(tic, random_number, indir, outpath, syspath, teff, srad, args)
	#print ("done.")
	...
	if os.path.exists("{}/pyaneti_LATTE.py".format(syspath)):
		...
		if model == True:
			...
			os.system("python3 {}/pyaneti_LATTE.py {} {} {} {} {} {} {}".format(syspath, tic, outpath, syspath, mstar, teff, srad, transit_list_model))
	...
	sectors_all = LATTEutils.get_sectors(tic)

	# Finally, create a DV report which summarises all of the plots and tables.

	# always create a LATTE rerport - that's the whole point of this

	if BLS == True:
		LATTEDV.LATTE_DV(tic, random_number, indir, outpath, syspath, transit_list, sectors_all, target_ra, target_dec, tessmag, teff, srad, mstar, vmag, logg, mass_ast, radius_ast, logg_ast, numax, deltanu, plx, c_id, bls_stats1, bls_stats2, tpf_corrupt, astroquery_corrupt, FFI = args.FFI,  bls = True, model = model, mpi = args.mpi)
	
	else:
		LATTEDV.LATTE_DV(tic, random_number, indir, outpath, syspath, transit_list, sectors_all, target_ra, target_dec, tessmag, teff, srad, mstar, vmag, logg, mass_ast, radius_ast, logg_ast, numax, deltanu, plx, c_id, [0], [0], tpf_corrupt, astroquery_corrupt, FFI = args.FFI,  bls = False, model = model, mpi = args.mpi)
...
def brew_LATTE_FFI(tic, indir, syspath, transit_list, simple, BLS, model, save, DV, sectors, sectors_all, alltime, allflux_normal, allflux_small, allflux, all_md, allfbkg, allfbkg_t, start_sec, end_sec, in_sec, X1_list, X4_list, apmask_list, arrshape_list, tpf_filt_list, t_list, bkg_list, tpf_list, ra, dec, upper_axis_lim_final, lower_axis_lim_final, args):
	...
	if (save == True) or (DV == True):
		...
		# if this folder doesn't exist then create it. These are the folder where the images, data and reports for each TIC ID will be stored.
		newpath = '{}/{}_rn{}'.format(indir,tic, random_number)

		if not exists(newpath):
			os.makedirs(newpath)

		# save the data used as a text file - these often come in use later for a quick re-analysis.
		with open('{}/{}_rn{}/{}_data.txt'.format(indir, tic, random_number, tic), "w") as f:
		...
		if model == True:
			with open('{}/{}_rn{}/{}_data_pyaneti.dat'.format(indir, tic, random_number, tic), "w") as f:
				writer = csv.writer(f, delimiter='\t')
				writer.writerow(['#time', 'flux', 'flux_err'])

			# if the dip separations are too small, then don't create cut outs and save the whole dataset
			if (len(transit_list) > 1) and ((transit_list[1] - transit_list[0]) < 2): # if there are LOTS of transit events on short period (if so it's probably a TOI but let's keep it here as a condition)
				with open('{}/{}_rn{}/{}_data_pyaneti.dat'.format(indir, tic, random_number, tic), "a") as f:
					writer = csv.writer(f, delimiter='\t')
					writer.writerows(zip(alltime_ar,allflux_ar,allflux_err_ar)) # save all the data
	...
	# create a plot of the fulllighcurves with the momentum dumps (MDs) marked and a zoom-in of the marked transits
	utils.plot_full_md(tic, random_number, indir, alltime, allflux, all_md, alltime, allflux, transit_list, upper_axis_lim_final, lower_axis_lim_final, args)
	...
	# plot the background flux at the time of the transit event.
	utils.plot_background(tic, random_number, indir, allfbkg_t, allfbkg, transit_list, args)
	...
	utils.plot_aperturesize(tic,random_number, indir,alltime, alltime, alltime, allflux_normal, allflux_normal, allflux_small, transit_list, args)
  ...
  # needs to know a period so can only do this if more than one transit has been marked.
	...
	if BLS == True:
		print ("Running BLS")
		bls_stats1, bls_stats2 = utils.data_bls_FFI(tic, random_number, indir, alltime, allflux, args)
	# ------------

	# ------------
	print ("Periodogram plot...", end =" ")
	mass_ast, radius_ast, logg_ast, numax, deltanu = utils.plot_periodogram(tic, random_number, indir, alltime, allflux, 0, -999, -999, args)

	print ("done.")

	# ------------
	print ("Evolutionary tracks plot...", end =" ")
	utils.eep_target(tic, random_number, indir, syspath, teff, srad, args)
	print ("done.")
	...
	if DV == True:
		...
		if BLS == True:
			ldv.LATTE_DV(tic, indir, outpath, syspath, transit_list, sectors_all, ra, dec, tessmag, teff, srad, mstar, vmag, logg, mass_ast, radius_ast, logg_ast, numax, deltanu, plx, c_id, bls_stats1, bls_stats2, False, astroquery_corrupt, FFI = True, bls = True, model = model, mpi = args.mpi)
		else:
			ldv.LATTE_DV(tic, indir, outpath, syspath, transit_list, sectors_all, ra, dec, tessmag, teff, srad, mstar, vmag, logg, mass_ast, radius_ast, logg_ast, numax, deltanu, plx, c_id, [0], [0], False, astroquery_corrupt, FFI = True, bls = False, model = model, mpi = args.mpi)
