...
def LATTE_DV(tic, random_number, indir, outpath, syspath, transit_list, sectors_all, target_ra, target_dec, tessmag, teff, srad, mstar, vmag, logg, mass_ast, radius_ast, logg_ast, numax, deltanu, plx, c_id, bls_stats1, bls_stats2, tpf_corrupt, astroquery_corrupt, FFI, bls = False, model = False, mpi = False, test = 'no'):
	...
	full_LC_name = "{}/{}_rn{}/{}_fullLC_md.png".format(outpath,tic,random_number,tic)

	full_LC_notransits_name = "{}/{}_rn{}/{}_fullLC_md_notransits.png".format(outpath,tic,random_number,tic) # only used in the 'asteroseismic only' version of LATTE

	background_flux_name = '{}/{}_rn{}/{}_background.png'.format(outpath, tic, random_number,tic)

	centroid_positions_name = '{}/{}_rn{}/{}_centroids.png'.format(outpath, tic,random_number, tic)

	flux_aperture_name= '{}/{}_rn{}/{}_aperture_size.png'.format(outpath, tic, random_number,tic)

	tess_stars_name = '{}/{}_rn{}/{}_star_field.png'.format(outpath, tic, random_number,tic)
	...
	bls2 = '{}/{}_rn{}/{}_bls_second.png'.format(outpath, tic, random_number,tic)

	in_out_name = '{}/{}_rn{}/{}_flux_comparison.png'.format(outpath, tic, random_number,tic)

	model_name = '{}/{}_rn{}/model_out/{}b_tr.png'.format(outpath, tic, random_number,tic)

	#phasefold_name = '{}/{}_rn{}/{}_phase_folded.png'.format(outpath, tic, random_number,tic)

	apertures_name = '{}/{}_rn{}/{}_apertures_0.png'.format(outpath, tic, random_number,tic)

	periodogram_name = '{}/{}_rn{}/{}_periodogram.png'.format(outpath, tic, random_number,tic)

	periodogram_numax_name = '{}/{}_rn{}/{}_diagnostic_numax.png'.format(outpath, tic, random_number,tic)

	periodogram_correlation_name = '{}/{}_rn{}/{}_scaled_correlation.png'.format(outpath, tic, random_number,tic)

	periodogram_echelle_name = '{}/{}_rn{}/{}_echelle.png'.format(outpath, tic, random_number,tic)

	eep_name = '{}/{}_rn{}/{}_eep.png'.format(outpath, tic, random_number,tic)

	# ----- LOGOS ------
	# if this is a unittest run, find the files for the logos stored in the test folder
	if test != 'no':
		PHT_logo_name  =  '{}/static/LATTE_imgs/PHT_logo.jpg'.format(test)
		LATTE_logo_name = '{}/static/LATTE_imgs/LATTE_logo.png'.format(test)
		TESS_logo_name  = '{}/static/LATTE_imgs/TESS_logo.png'.format(test)

	# otherwise they're located in the place where the program is insatlled.
	else:
		PHT_logo_name  =  '{}/static/LATTE_imgs/PHT_logo.jpg'.format(syspath)
		LATTE_logo_name = '{}/static/LATTE_imgs/LATTE_logo.png'.format(syspath)
		TESS_logo_name  = '{}/static/LATTE_imgs/TESS_logo.png'.format(syspath)
	...
	doc = SimpleDocTemplate("{}/{}_rn{}/DV_report_{}.pdf".format(outpath,tic,random_number,tic) ,pagesize=A4,
	...
	extended_mission_sectors = list(np.array(sectors_all)[(np.array(sectors_all) > 26) & (np.array(sectors_all) < 53)])
	further_extended_mission_sectors = list(np.array(sectors_all)[np.array(sectors_all) > 52])
	...
	if len(transit_list) != 0: # normal mode
		...
		if tpf_corrupt == False:
			...
			#see what pixel level LC plots are available (might be one or more)

			pixel_LCs_name_list = glob('{}/{}_rn{}/{}_individual_pixel_LCs_*.png'.format(outpath, tic, random_number, tic))


			for i,pixel_LCs_name in enumerate(pixel_LCs_name_list):
				...
				# only append the figure text on the last one of the plots
				if i == 0:
					fig_count += 1
	...
	if model == True:
		...
		if os.path.exists("{}/{}_rn{}/model_out/{}_parameters.csv".format(indir, tic, random_number,tic)):
			...
			manifest_table = pd.read_csv('{}/{}_rn{}/model_out/{}_parameters.csv'.format(indir, tic, random_number,tic))
